...
class AugmentedMemoryConvTransformerEncoder_fill_sum_leftV2(ConvTransformerEncoder):
    ...
    def add_left_context(self, memory, input, mem_size, src_lengths):
        if mem_size > 0:
            left_context = torch.cat(memory, 0)
...
class AugmentedMemoryMultiheadAttention(MultiheadAttention):
    ...
    def __init__(
        ...
        self.summarize = torch.nn.AdaptiveAvgPool1d(1)
    ...
    def add_memory(self, memory):
        new_memory = []
        if len(memory) > 0:
            combined_memory = torch.cat(memory, 0)
            if self.mem_bank_start != 0:
                combined_memory = combined_memory[:-self.mem_bank_start]
            mem_size = combined_memory.size(0)
            pos = mem_size
            if mem_size % self.max_memory_size == 0:
                bank_size = mem_size // self.max_memory_size
                for i in range(self.max_memory_size):
                    new_memory = self.compress_memory(combined_memory, new_memory, pos, bank_size)
                    pos = pos - bank_size
            else:
                zp = self.max_memory_size - (mem_size % self.max_memory_size)
                pp = mem_size // self.max_memory_size
                for i in range(self.max_memory_size):
                    if i >= zp:
                        new_memory = self.compress_memory(combined_memory, new_memory, pos, pp+1)
                        pos = pos - (pp + 1)
                    else:
                        new_memory = self.compress_memory(combined_memory, new_memory, pos, pp)
                        pos = pos - pp

        return new_memory

    def compress_memory(self, combined_memory, new_memory, pos, bank_size):
        bank = combined_memory[pos-bank_size:pos].transpose(0,2)
        bank = self.summarize(bank)
        bank = bank.transpose(0,2)
        if new_memory != []:
            new_memory = torch.cat([bank] + [new_memory], 0)
        else:
            new_memory = bank
        return new_memory
...
#   SequenceEncoder
# ------------------------------------------------------------------------------
class SequenceEncoder_fill_sum_leftV2(FairseqEncoder):
    """
...
# ------------------------------------------------------------------------------
def augmented_memory_fill_sum_leftV2(klass):
