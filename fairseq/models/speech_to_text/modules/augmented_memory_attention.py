...
class AugmentedMemoryConvTransformerEncoder(ConvTransformerEncoder):
    ...
    def add_memory(self, memory, input, src_lengths, old_left_context_size):
        ...
        if mem_size > 0 and self.left_context_after_stride != 0:
            ...
            if old_left_context_size != 0 and old_left_context_size < left_context_size:
                left_context = left_context[:-old_left_context_size]
            elif old_left_context_size != 0 and old_left_context_size >= left_context_size:
                print("old_left_context_size: ", old_left_context_size)
                print("input_size: ", input.size(0))
                print("left_context_size", left_context_size)
                input = input[self.left_context_after_stride:]
                left_context = left_context[:left_context_size-self.left_context_after_stride]
        ...
        for seg_src_tokens, seg_src_lengths in seg_src_tokens_lengths:
            ...
            if self.right_context != 0:
                ...
                if total_seg_count > cur_seg_count+1:
                    ...
                    elif right_context_size % 4 != 0 and (left_context_size > 0 or self.left_context > 0):
