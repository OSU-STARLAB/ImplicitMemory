...
class AugmentedMemoryConvTransformerEncoder_layer_mem_shrink(ConvTransformerEncoder):
    def __init__(self, args):
        ...
        self.max_token_count = args.max_token_count // self.encoder_stride
    ...
    def add_memory(self, memory, input, mem_size, src_lengths):
        if mem_size > 0:
            ...
            if self.left_context_after_stride < left_context_size:
                if self.shrink_left_context:
                    left_context = self.shrink_left(memory, mem_size)
                else:
    ...
    def shrink_left(self, memory, mem_size):
        new_left = []
        for i in range(mem_size):
            if mem_size > i + 1:
                summarize = torch.nn.AdaptiveAvgPool1d(self.left_context_after_stride//2**(i+1))
            else:
                summarize = torch.nn.AdaptiveAvgPool1d(self.left_context_after_stride//2**i)
            segment = memory[mem_size - i - 1].transpose(0,2)
            segment = summarize(segment)
            segment = segment.transpose(0,2)
            if new_left == []:
                new_left = segment
            else:
                new_left = torch.cat([segment] + [new_left], dim=0)
        return new_left
...
class AugmentedMemoryTransformerEncoderLayer(TransformerEncoderLayer):
    ...
    def forward(self, x, state, layer_num, left_context):
...
#   SequenceEncoder
# ------------------------------------------------------------------------------
class SequenceEncoder_layer_mem_shrink(FairseqEncoder):
    """
    ...
    def __init__(self, args, module):
        ...
        self.left_context_method = args.left_context_method
        self.max_token_count = args.max_token_count
        self.max_segment_count = ceil(self.max_token_count / self.segment_size)
        self.shrink_left_context = getattr(args, "shrink_left_context", False)
    ...
    def shrink_left(self, memory, mem_size):
        ...
        for i in range(mem_size):
            if mem_size > i + 1:
                summarize = torch.nn.AdaptiveAvgPool1d(self.left_context//2**(i+1))
            else:
                summarize = torch.nn.AdaptiveAvgPool1d(self.left_context//2**i)
            segment = memory[mem_size - i - 1].transpose(self.input_time_axis,2)
            segment = summarize(segment)
            segment = segment.transpose(self.input_time_axis,2)
            if new_left == []:
                new_left = segment
            else:
                new_left = torch.cat([segment] + [new_left], dim=self.input_time_axis)
        return new_left
...
# ------------------------------------------------------------------------------
def augmented_memory_layer_mem_shrink(klass):
