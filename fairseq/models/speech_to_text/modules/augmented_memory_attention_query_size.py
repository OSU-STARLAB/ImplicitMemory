...
class AugmentedMemoryConvTransformerEncoder_query_size(ConvTransformerEncoder):
    def __init__(self, args):
        ...
        self.sum_query_size = getattr(args, "sum_query_size", 1)
    ...
    def get_relative_position(
        ...
        query_ranges = torch.arange(0, seq_len+self.sum_query_size)
...
class AugmentedMemoryTransformerEncoderLayer(TransformerEncoderLayer):
    ...
    def forward(self, x, state, layer_num, rpe, new_left_context):
        ...
        x = self.self_attn(input_and_summary=x, state=state, layer_num=layer_num, rpe=rpe, new_left_context=new_left_context)
    ...
    def build_self_attention(self, embed_dim, args):
        return AugmentedMemoryMultiheadAttention(
            ...
            sum_query_size = getattr(args, "sum_query_size", 1),
            increase_context=getattr(args, "increase_context", False),
...
class AugmentedMemoryMultiheadAttention(MultiheadAttention):
    ...
    def update_mem_banks(self, state, output_and_memory, layer_num):
        if self.share_mem_bank_layers is not None:
          if not any(layer_num in layer for layer in self.share_mem_bank_layers):
            banks = output_and_memory[-self.sum_query_size:]
            for i in range(self.sum_query_size):
                index = torch.tensor([i], device=banks.device)
                next_m = torch.index_select(banks, 0, index)
    ...
    def forward(self, input_and_summary, state, layer_num, rpe, new_left_context):
        """
